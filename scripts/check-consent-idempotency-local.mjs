...
assert.match(migration,/CREATE UNIQUE INDEX consents_active_policy_version_uq/i);assert.match(migration,/profile_id, consent_type, policy_version/i);assert.match(migration,/WHERE granted = true AND revoked_at IS NULL/i);assert.match(migration,/consent_type IN \('terms','privacy'\)/i);
...
console.log('OK: aceite jurídico e consentimentos ativos são atômicos e idempotentes sob concorrência');
