...
export class PostgresAcquisitionInsights {
  ...
  async dashboard(days=30){
    ...
    const [totals,daily,sources,campaigns,consents,rate]=await Promise.all([
      this.pool.query(`select count(*)::int leads,
        count(*) filter(where occurred_at>=now()-interval '24 hours')::int last_24h,
      ...
      this.pool.query(`select date_trunc('day',occurred_at)::date day,count(*)::int leads
        from analytics_events where event_name='lead_created' and occurred_at>=now()-($1::text||' days')::interval
        group by 1 order by 1`,[days]),
      this.pool.query(`select source,count(*)::int leads,count(distinct profile_id)::int people
      ...
      this.pool.query(`select coalesce(campaign,'(sem campanha)') campaign,count(*)::int leads,count(distinct profile_id)::int people
        from acquisition_attributions where occurred_at>=now()-($1::text||' days')::interval
      ...
      select consent_type,count(*)::int total,count(*) filter(where granted)::int granted,
        round(100.0*count(*) filter(where granted)/nullif(count(*),0),2) granted_pct
      from latest group by consent_type order by consent_type`),
      this.pool.query(`select coalesce(sum(hit_count),0)::int hits,count(*)::int buckets,max(updated_at) last_hit_at
