import type { PaymentMethod, RawWebhookRequest, VerifiedWebhook } from 'https://raw.githubusercontent.com/squalmg/diretoria/c0fefccf0cf71b664ed6860b595dbe1bb037b827/packages/payments/provider-contract.ts';
...
function normalizedPaymentEvent(payment: any): VerifiedWebhook['eventType'] {
  ...
  // Pix CONFIRMED pode estar sob bloqueio cautelar; somente RECEIVED é liquidação definitiva.
  if (method === 'pix') {
    if (status === 'RECEIVED') return 'paid';
    if (['CONFIRMED','PENDING','AWAITING_RISK_ANALYSIS'].includes(status)) return 'pending';
  }
  // Cartão CONFIRMED representa pagamento concluído; RECEIVED pode ocorrer muito depois.
  if (method === 'card') {
